...
De plus, il y deux modes de jeu: le mode standard, et un mode "chaos" avec des évènements aléatoires. Je vous renvoie au sujet de maths que vous donnera Mr. Martinez pour plus d'infos sur ces évènements aléatoires que vous devrez implémenter.
...
- Ce projet se fera par groupes de 2 (et *exceptionnellement* il peut y avoir un groupe de 3 ou de 1. Il faut obligatoirement venir nous en parler dès le début du semestre pour qu'on valide ça ensemble.)
...
**Il suffira de me rendre un lien vers votre repo git**, j'irai lire votre code pour vous évaluer.<br/>
Il y aura aussi une soutenance, mais je l'utilise assez peu pour vous évaluer, donc concentrez votre soutenance sur la partie synthèse d'image du projet.
...
Puis vous ferez une visualisation en 3D du plateau et des pièces (cf. le sujet de synthèse d'image), mais même après ça vous garderez la GUI pour jouer les mouvements des pièces, et le monde 3D reflétera les changements. (Puis dans un deuxième temps, on vous demande dans le sujet de synthèse d'image de pouvoir cliquer directement sur les pièces dans le monde 3D pour les bouger)
