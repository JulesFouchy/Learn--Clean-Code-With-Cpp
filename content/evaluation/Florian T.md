...
```cpp
  int find_biome(
      std::vector<int>
          &tableau_de_biomes)
```
Passer par const& si vous ne modifiez pas le tableau.

```cpp
  void assign_DND_alignement(std::vector<int> &tableau_de_biomes) {
```
C'est quoi DND ? Évitez les abbreviations, ça rend votre code difficile à comprendre.
...
```
Plutôt qu'une fonction clear, faites un destructeur. Il sera appelé automatiquement c'est vachement pratique.