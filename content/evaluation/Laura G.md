...
```
Vous pouvez faire un enum pour ça :
```cpp
enum class BoidType{
    Fish,
    Star,
...
Énormément de code dupliqué dans votre `main()` pour la création des objets OpenGL. Vous auriez pu faire une classe qui aurait fait tout ça automatiquement.